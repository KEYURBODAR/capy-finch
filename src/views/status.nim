...
import tweet, timeline, actions, renderutils
...
proc renderStatusPage*(conv: Conversation; prefs: Prefs; path: string): VNode =
  let
    basePath = getLink(conv.tweet, focus=false)
    articleLink =
      if conv.tweet.article.isSome or getArticleUrl(conv.tweet).len > 0:
        basePath & "/article"
      else:
        ""

  buildHtml(tdiv(class="timeline-container status-page")):
    tdiv(class="timeline-header timeline-header-left"):
      renderPageActions([
        ("JSON", basePath & "/json"),
        ("MD", basePath & "/md"),
        ("TXT", basePath & "/txt"),
        ("LIVE", basePath & "/live/json"),
        ("Article", articleLink)
      ])
      renderPageMeta("Stored content is retained for 45 days. Use LIVE for current engagement and availability.")

    renderConversation(conv, prefs, path)
